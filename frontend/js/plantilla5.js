...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("presentacionBtn").addEventListener("click", () => {
    ...
    // === GALERÍA FLIP TIPO RULETA ===
function renderRuleta(jugadores) {
    ...
    <div class="flip-card">
      <div class="flip-card-inner">
        ...
        <div class="flip-card-back">
          <h4>Estadísticas</h4>
          <p>Horas jugadas: ${jugador.horas || 'N/A'}</p>
          <p>Streamer: ${jugador.streamer ? "✅" : "❌"}</p>
          <a href="${jugador.tracker}" target="_blank">Ver Tracker</a>
        </div>
      </div>
    </div>
  `;

  const renderizarActual = () => {
